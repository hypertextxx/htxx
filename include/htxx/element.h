#ifndef HTXX_ELEMENT_H
#define HTXX_ELEMENT_H
...
namespace detail {
    template <std::size_t N> constexpr auto index_range = std::make_index_sequence<N>{ };
...
template <class T, std::meta::info Gen> constexpr bool permits_aspect() {
    return true;
    
    /*bool permits_default = true;
    template for (constexpr auto ann : std::meta::annotations_of(^^T)) {
        constexpr auto ann_type = type_of(ann);
        if constexpr (has_template_arguments(ann_type) && template_of(ann_type) == ^^permit) {
            permits_default = false;
            template for (constexpr auto gen_type : std::meta::template_arguments_of(ann_type)) {
                if (is_type(gen_type) && gen_type == Gen) {
                    return true;
                }
            }
        }
    }
    return permits_default;*/
}
...
template <template <class...> class El, class... A> class element<El<A...>>: public with_aspects<A...>,
    ...
    template <class Gen> static consteval auto aspect_info() {
        ...
        static constexpr auto storage_candidates = aspect_storage_types();
        ...
        template for (constexpr std::meta::info base : storage_candidates) {
            ...
            static constexpr auto mems = define_static_array(nonstatic_data_members_of(storage, ctx));
...
#pragma GCC diagnostic ignored "-Wsfinae-incomplete"
    static consteval auto children_in() {
        ...
        template for (constexpr std::meta::info base : aspect_storage_types<el_type>()) {
            static constexpr auto mems = define_static_array(nonstatic_data_members_of(base, ctx));
            template for (constexpr auto mem : mems) {
...
#pragma GCC diagnostic pop
    ...
    template <std::size_t I> static consteval std::size_t child_index_from_raw_index() {
        std::size_t c = I;
        template for (constexpr auto J : detail::index_range<I>) {
    ...
    template <class T, std::size_t I> constexpr void initialize(T&& t) {
        ...
        if constexpr (is_aspect_type(info)) {
            ...
            if constexpr (!detail::permits_aspect<el_type, generator_info>()) {
                throw impermissible_aspect<el_type, typename[:generator_info:]>{ };
            }
...
public:
    ...
    static constexpr std::size_t count_children() { return children_in().size(); }
